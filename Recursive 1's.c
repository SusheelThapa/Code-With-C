...
/*So, we come to the heart of the problem. Recursion basically means to call a function from within itself. As programmers, recursion will play a big role in what we do later on so I'll try to give you a step by step approach that worked for me.

For a recursive problem there are basically three steps:

The first step is to think of a "base case". This is best visualized by thinking of the simplest input to the function. In our case, it will be sum(1) = 1.

The second step is to think how the base case can be used to solve the larger problem. It will be helpful to focus on a single like 1 or 2 and try to come up with a recursive function that gives us the desired output.

The third step is to generalize this function for other terms so that we can solve the problem.
 */
...
/*In the above function we set our base case to be when n = 1. When our base case occurs we want to return 10^0 = 1. This is also the condition that will prevent the recursive call from running indefintely. 
What if we don't encounter our base case? Now this is the tricky part. This is where you have to use the recursive function that you thought of. Note that the term n will decrease with every recursive call.
